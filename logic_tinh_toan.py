...
def lay_ti_le_don_hang(farmer_id):
    """
    Tính tỉ lệ đơn hàng theo sản phẩm (dựa trên số lượng OrderItems)
    """
    conn = get_connection()
    cursor = conn.cursor()

    try:
        # Tổng số lượng sản phẩm trong các đơn hàng hoàn thành
        cursor.execute("""
            SELECT SUM(oi.quantity) 
            FROM Orders o
            JOIN OrderItems oi ON o.order_id = oi.order_id
            WHERE o.farmer_id = ? AND o.status = 'Hoàn thành'
        """, (farmer_id,))
        ...
        # Lấy số lượng theo từng loại sản phẩm
        cursor.execute("""
        ...
        print(f"Lỗi hàm lay_ti_le_don_hang: {e}")
        return None
    finally:
        conn.close()
