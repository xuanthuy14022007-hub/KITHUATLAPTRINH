...
from logic.trade import lay_chi_tiet_don_hang, cap_nhat_trang_thai_don_hang
...
class ChiTietDonHangScreen(QWidget):
    def __init__(self, order_id):
        ...
        self.product_labels = []
        for row in range(1, 6):  # 5 dòng
            row_labels = []
            for col in range(1, 5):  # 4 cột
    ...
    def load_data(self):
        """Tải chi tiết đơn hàng từ backend và hiển thị."""
        try:
            details = lay_chi_tiet_don_hang(self.order_id)
        except Exception as e:
            QMessageBox.critical(self, "Lỗi", f"Không thể tải chi tiết đơn: {str(e)}")
            self.quay_lai_danh_sach()
            return

        if not details:
            QMessageBox.warning(self, "Thông báo", "Không có dữ liệu chi tiết cho đơn hàng này.")
            self.quay_lai_danh_sach()
            return
        ...
        # Hiển thị tổng khối lượng và tổng giá trị
        ...
        # TODO: Nếu backend có thêm thông tin khách hàng, cập nhật các label tương ứng
        # Ví dụ:
        # if self.lbl_ten_kh:
        ...
        # if self.lbl_desc_kh:
        #     self.lbl_desc_kh.setText(f"#{self.order_id} - {order_date}")
        # if self.lbl_avatar_kh:
        #     # set pixmap nếu có
        #     pass
