...
class GioHangScreen(QWidget):
    def __init__(self):
        ...
        for i in range(1, 4):
            ...
            if btn_m:
                btn_m.clicked.connect(lambda _, idx=i: self.thay_doi_so_luong(idx, -1))
            if btn_p:
                btn_p.clicked.connect(lambda _, idx=i: self.thay_doi_so_luong(idx, 1))
            if txt_q:
                txt_q.textChanged.connect(lambda _, idx=i: self.cap_nhat_thanh_tien(idx))
            if chk:
                chk.stateChanged.connect(self.cap_nhat_tong_tien)

        self._don_gia = {}  # Lưu đơn giá từng item
    ...
    def tai_du_lieu_gio_hang(self):
        ...
        item_configs = [
            {'frame': 'frame_item_1', 'lbl_name': 'lbl_name_1', 'lbl_rem': 'lbl_rem_1',
             'lbl_p': 'lbl_p_val_1', 'txt_q': 'txt_q_1', 'lbl_t': 'lbl_t_val_1', 'chk': 'chk_item_1'},
            {'frame': 'frame_item_2', 'lbl_name': 'lbl_name_2', 'lbl_rem': 'lbl_rem_2',
             'lbl_p': 'lbl_p_val_2', 'txt_q': 'txt_q_2', 'lbl_t': 'lbl_t_val_2', 'chk': 'chk_item_2'},
            {'frame': 'frame_item_3', 'lbl_name': 'lbl_name_3', 'lbl_rem': 'lbl_rem_3',
             'lbl_p': 'lbl_p_val_3', 'txt_q': 'txt_q_3', 'lbl_t': 'lbl_t_val_3', 'chk': 'chk_item_3'},
        ]
        for i, cfg in enumerate(item_configs):
            ...
            item = gio_hang[i]
            # item: (cart_id, crop_name, quantity, price, ton_kho, activity_id)
            ...
            if hasattr(self, cfg['lbl_p']):
                getattr(self, cfg['lbl_p']).setText(
                    f'<html><body><span style="font-weight:bold; font-size:13pt;">{don_gia:,.0f}</span>'
                    f'<span style="font-size:11pt; color:#4A4A4A;"> VND / kg</span></body></html>'
                )
            ...
            if hasattr(self, cfg['lbl_t']):
                getattr(self, cfg['lbl_t']).setText(
                    f'<html><body><span style="font-weight:bold; font-size:13pt;">{thanh_tien:,.0f}</span>'
                    f'<span style="font-size:11pt; color:#4A4A4A;"> VND</span></body></html>'
                )
