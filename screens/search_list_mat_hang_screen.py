...
class SearchListMatHangScreen(QWidget):
    ...
    def tai_du_lieu(self):
        ...
        # Cập nhật số lượng (nếu có label)
        ...
        for i in range(1, 7):
            ...
            if i - 1 < len(danh_sach):
                sp = danh_sach[i - 1]
                card.show()
                # sp = (activity_id, full_name, crop_name, quantity, price, plot_name)
                ...
                # Lưu activity_id vào card và gắn sự kiện
                card.activity_id = sp[0]
                # Sử dụng lambda với tham số mặc định để tránh binding muộn
                card.mousePressEvent = lambda event, aid=card.activity_id: self.mo_chi_tiet_nong_san(aid)
            else:
                card.hide()

    def mo_chi_tiet_nong_san(self, activity_id):
        from screens.chi_tiet_nong_san_screen import ChiTietNongSanScreen
        switch_window(ChiTietNongSanScreen, activity_id=activity_id)
