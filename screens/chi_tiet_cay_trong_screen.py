...
from logic.logic_mua_vu import lay_chi_tiet_vu_mua
...
class ChiTietCayTrongScreen(QWidget):
    def __init__(self, activity_id=None):
        ...
        if hasattr(self, 'lbl_xem_nhat_ky'):
            self.lbl_xem_nhat_ky.mousePressEvent = self.mo_nhat_ky
    ...
    def tai_du_lieu(self):
        ...
        activity_id, crop_name, plot_name, area_m2, start_date, selling_price, status = vu_mua[:7]
        # Lấy mô tả từ lay_chi_tiet_vu_mua
        chi_tiet = lay_chi_tiet_vu_mua(activity_id)
        mo_ta = chi_tiet[8] if chi_tiet and len(chi_tiet) > 8 and chi_tiet[8] else ''
        ...
        if hasattr(self, 'img_cay_trong'):
            self.img_cay_trong.setText(icon)
        if hasattr(self, 'lbl_ten_cay_to'):
            self.lbl_ten_cay_to.setText(crop_name)
        if hasattr(self, 'lbl_badge_thua'):
            self.lbl_badge_thua.setText(f"Thửa đất: {plot_name}")
        if hasattr(self, 'lbl_mo_ta'):
            self.lbl_mo_ta.setText(mo_ta if mo_ta else "Chưa có mô tả.")
        ...
        if hasattr(self, 'lbl_ttc_col2_1'):
            self.lbl_ttc_col2_1.setText(crop_name)
        if hasattr(self, 'lbl_ttc_col2_2'):
            self.lbl_ttc_col2_2.setText(plot_name)
        if hasattr(self, 'lbl_ttc_col2_3'):
            self.lbl_ttc_col2_3.setText(area_str)
        if hasattr(self, 'lbl_ttc_col2_4'):
            self.lbl_ttc_col2_4.setText(start_date or "")
        if hasattr(self, 'lbl_badge_canhbao'):
            self.lbl_badge_canhbao.setVisible(False)
        if hasattr(self, 'prg_sinh_truong'):
            self.prg_sinh_truong.setValue(prg)
